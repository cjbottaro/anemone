...
The crawl is finished when all processes exit.  Each process will exit when the link_queue and page_queue have been empty for +finish_timeout+ seconds (which is an Anemone option unique to this fork).
